...
class RvparkyLocationValidatorTest < ActiveSupport::TestCase
  ...
  test 'validator should set up properly' do
    ...
    assert @validator.former == @params[:formerlyKnownAs]
    assert @validator.weekly_rate_updated == @params[:weeklyRateUpdated]
    assert @validator.rating == @params[:rating]
    assert @validator.offseason_daily_rate == @params[:dailyRate_offseason]
    assert @validator.known_as == @params[:alsoKnownAs]
    assert @validator.daily_rate == @params[:dailyRate]
    ...
    assert @validator.postal_code == @params[:zip_code]
    assert @validator.phone == @params[:phone_number]
    assert @validator.open_dates == @params[:open_dates]
    assert @validator.seasonal_rates == @params[:seasonalRates]
    assert @validator.description == @params[:description]
    assert @validator.offseason_date == @params[:offSeasonDates]
    assert @validator.address == @params[:address]
    assert @validator.slug == @params[:slug]
    assert @validator.inseason_date == @params[:inSeasonDates]
    assert @validator.name == @params[:name]
    assert @validator.daily_rate_updated == @params[:dailyRateUpdated]
    assert @validator.monthly_rate_updated == @params[:monthlyRateUpdated]
    assert @validator.region == @params[:region]
    assert @validator.monthly_rate == @params[:monthlyRate]
    assert @validator.weekly_rate == @params[:weeklyRate]
    assert @validator.offseason_weekly_rate == @params[:weeklyRate_offseason]
    assert @validator.offseason_monthly_rate == @params[:monthlyRate_offseason]
